...
+ (MBProgressHUD *)showHUDWithCancelAddedTo:(UIView *)view animated:(BOOL)animated andController:(id)pController
{
    MBProgressHUD *hud = [[MBProgressHUD alloc] initWithViewForCancellation:view
                                                              andController:pController];
...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view withLabel:(NSString *)text animated:(BOOL)animated
{
    MBProgressHUD *hud = [[self alloc] initWithView:view];
...
- (id)initWithFrame:(CGRect)frame andCancel:(BOOL)yesORNo andController:(id)pControl
{
    ...
    if (self)
    {
        ...
        self.allowsCancelation = yesORNo;
        self.controller = pControl;
...
- (id)initWithViewForCancellation:(UIView *)view andController:(id)control
{
    NSAssert(view, @"View must not be nil.");
    id me = [self initWithFrame:view.bounds
                      andCancel:TRUE
                  andController:control];
...
- (BOOL)shouldPerformOrientationTransform
{
    ...
    return isPreiOS8 && [self.superview
...
- (void)showUsingAnimation:(BOOL)animated
{
    ...
    if (animated && animationType == MBProgressHUDAnimationZoomOut)
    {
...
{
    if ([self.controller
         respondsToSelector:@selector(cancelCurrentTask)])
    {
//        [self.controller cancelCurrentTask];
    }

//   [(AbstractViewController *)self.controller cancelCurrentTask];
}
...
- (void)updateIndicators
{
    ...
    if (mode == MBProgressHUDModeIndeterminate)
    {
        ...
        [(UIActivityIndicatorView *)indicator
         setColor:self.activityIndicatorColor];
#endif
    }
    else
    ...
    if (mode == MBProgressHUDModeDeterminate || mode == MBProgressHUDModeAnnularDeterminate)
    {
        ...
        {
            [(MBRoundProgressView *)indicator
             setAnnular:YES];
        }
    }
    else
    if (mode == MBProgressHUDModeCustomView && customView != indicator)
    {
...
- (void)layoutSubviews
{
    ...
    if (self.allowsCancelation && self.controller != nil)
    {
        if (!self._cancelButton)
        {
            self._cancelButton = [UIButton buttonWithType:UIButtonTypeCustom];
            [self._cancelButton
             setImage:[UIImage imageNamed:@"cancel_btn_Hud"]
        ...
        self._cancelButton.frame = CGRectMake((self.bounds.size.width + self.size.width) / 2 + self.xOffset - 15,
                                              (self.bounds.size.height - self.size.height) / 2 + self.yOffset - 15,
                                              30, 30);
...
- (void)updateUIForKeypath:(NSString *)keyPath
{
    ...
    if ([keyPath isEqualToString:@"detailsLabelText"])
    {
        detailsLabel.text = self.detailsLabelText;
    }
    else
    if ([keyPath isEqualToString:@"detailsLabelFont"])
    {
        detailsLabel.font = self.detailsLabelFont;
    }
    else
    if ([keyPath isEqualToString:@"detailsLabelColor"])
    {
        detailsLabel.textColor = self.detailsLabelColor;
    }
    else
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        _progress = 0.f;
        _annular = NO;
        _progressTintColor = [[UIColor alloc] initWithWhite:1.f
                                                      alpha:1.f];
        _backgroundTintColor = [[UIColor alloc] initWithWhite:1.f
                                                        alpha:.1f];
        [self registerForKVO];
    }
    return self;
